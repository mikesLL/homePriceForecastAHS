...
X_inf_exp = .01*ones( size(X_apr )) ;  % inflation expecations
X_inf_act = .01*ones( size(X_apr )) ; % inflation realization
...
for t_use = (t_begin + h_step + 1) : t_end
    ...
    % home price process
    y_mean = y_ds.fore_combo(t_begin:t_use, i_combo_use) + X_rp(t_begin:t_use) - param.tau - X_inf_exp;
    y_act = y_ds.RET_fut(t_begin:t_use) + X_rp(t_begin:t_use ) - param.tau - X_inf_act;
    y_resid = y_act - y_mean;
    
    % stock return process
    spy_mean = mean(X_spy_ret)*ones(size(y_mean)) - X_inf_exp;
    spy_act = X_spy_ret(t_begin:t_use) - X_inf_act;
    spy_resid = spy_act - spy_mean;
    
    % mortgage process
    mort_mean = X_apr(t_begin:t_use) - X_inf_exp;    % observed mortgage rate - expected inflation
    mort_act = X_apr(t_begin:t_use) - X_inf_act;    % observed mortgage rate - actual inflation
    mort_resid = mort_act - mort_mean;    
    
    % tbill process
    tbill_mean = X_apr(t_begin:t_use) - X_inf_exp;   % tfor now: assume tbill rate is around morgage rate; tbill rate - expected inflation
    tbill_act = X_apr(t_begin:t_use) - X_inf_act;    % tbill rate - realized inflation
