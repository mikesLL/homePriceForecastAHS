...
port_ret_store0{5}{17} = zeros(100,1);
port_ret_store1{5}{17} = zeros(100,1);
port_ret_store2{5}{17} = zeros(100,1);

X_opt_store0{5}{17} = zeros(100,4);
X_opt_store1{5}{17} = zeros(100,4);
X_opt_store2{5}{17} = zeros(100,4);
...
for city_id = 1:N_cities
   ...
   %%
   port_ret_store0{g_i}{city_id} = port_ret0;
   port_ret_store1{g_i}{city_id} = port_ret1;
   port_ret_store2{g_i}{city_id} = port_ret2;
   
   X_opt_store0{g_i}{city_id} = x_opt_store0;
   X_opt_store1{g_i}{city_id} = x_opt_store1;
   X_opt_store2{g_i}{city_id} = x_opt_store2;
   ...
   table44.cer(city_id) = ( table44.mu1(city_id) - table44.mu0(city_id) ) - mv_gamma/2*( table44.std1(city_id).^2 - table44.std1(city_id).^2 );
   ...
   if true
       ...
       plot(date_axis(49:96),port_ret_store1{g_i}{city_id}(49:96),'--');
       plot(date_axis(49:96),port_ret_store2{g_i}{city_id}(49:96),':');
   
       legend('w/o MD', 'w/MD', 'w/o H');
       title_str0 = strcat(dsreadin_codes.city_str(city_id),' Portfolio Returns');
       title(title_str0);
       
       % plot weights with ability to take housing positions
       figure; hold on;
       plot(date_axis(49:96),X_opt_store0{g_i}{city_id}(49:96,:));
       plot(date_axis(49:96),X_opt_store1{g_i}{city_id}(49:96,:),'--');
       plot(date_axis(49:96),X_opt_store2{g_i}{city_id}(49:96,:),':');
      
       title_str2 = strcat(dsreadin_codes.city_str(city_id),' Portfolio Weights');
       title(title_str2);
       legend('X', 'H', 'M', 'RF');
