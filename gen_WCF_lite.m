...
function [ WCF, N_dcfp, N_cfp ] = gen_WCF_lite(param, a2_ds )
...
N_dcfp = sum(dcfp);
N_cfp = sum( rent >= PMT );            % simple: cash-flow positive at the outset

%dcfp_prop = sum(dcfp)/length(a2_ds);

WCF(dcfp) = WCF(dcfp) + MBAL(dcfp) ;
WCF(dcfn) = 14.0*param.P_HR;

WCF = max( WCF, .5*delta1_mult*param.P_HR );
